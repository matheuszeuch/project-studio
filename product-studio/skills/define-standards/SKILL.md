...
3. **Constitution.** Monte `constitution.md` a partir de `references/constitution-template.md`. Revise cada artigo com o usuário: manter, ajustar ou remover. Artigos são poucos e inegociáveis; preferências vão para o `CLAUDE.md`.
...
6. **Esqueleto do repositório** (Claude Code). O `engineering-lead` cria o projeto vazio com a stack, lint, formatação, testes (um teste de exemplo), CI e `.product-studio.json`. O CI precisa passar.
7. **CLAUDE.md.** Atualize os comandos (instalar, rodar, testar, lint, deploy) e o glossário resumido.
8. Encerre com a skill `gate-review` para o G4.
