---
name: product-method
description: Orquestra o método product-studio de criação de produtos (Kickoff → Design Thinking → Working Backwards → DDD → Padrões → Spec-Driven → AI-Driven Development → Brochures). Use quando o usuário disser "começar um app novo", "novo produto", "em que fase estamos", "qual o próximo passo", "status do projeto" ou "preparar o repositório".
---
...
As skills transversais são `gate-review`, para fechar qualquer portão, e `retro`, para melhorar o próprio método ao fim de cada fase.
...
É a fonte de verdade do andamento. Toda skill de fase lê o arquivo ao começar e o atualiza ao terminar. Modelo em `references/status-template.md`. Os hooks do Claude Code dependem do formato dos checkboxes dos portões (`- [x] G4`), então preserve esse formato.
...
6. Crie `.product-studio.json` com as pastas de código de produção (usadas pelo hook de guarda). Padrão: `{"productionPaths": ["src/", "supabase/", "tests/", "e2e/"]}`.
...
8. **Voltar é permitido.** Se uma fase invalidar a anterior, volte, registre o motivo no `STATUS.md` e reabra o portão.
